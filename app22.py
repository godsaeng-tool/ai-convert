...
# OpenAI API 키 설정
OPENAI_API_KEY = "키 입력해 주세요(추후 환경변수로 빼놓겠습니다)"
...
DATA_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data')  # 강의 텍스트 저장용 폴더 추가
...
os.makedirs(DATA_FOLDER, exist_ok=True)  # 강의 텍스트 저장용 폴더 생성
...
# 벡터 디비 생성
vector_index_db = faiss.IndexFlatL2(1536)
...
def send_to_ai_processor(task_id, audio_path):
    try:
        ...
        text_path = os.path.join(app.config['DATA_FOLDER'], f"{task_id}.txt")
...
def generate_quiz(task_id, summary_text):
    ...
    try:
        ...
        update_progress(task_id, "completed", 100, "처리 완료", {"quiz": quiz_text})
        return quiz_text
...
def process_lecture(task_id, file_path=None, url=None):
    ...
    try:
        ...
        # 7. 임시 파일 정리 (파일 저장 안 하고, 처리 완료하면 삭제하는 방식으로)
...
def summary_api():
    ...
    if request.method == 'POST':
        ...
        if not os.path.exists(os.path.join(app.config['DATA_FOLDER'], f"{lecture_id}.txt")):
            return jsonify({"error": f"lecture_id {lecture_id}에 해당하는 데이터를 찾을 수 없습니다."}), 404
        
        if streaming:
            # 스트리밍 방식 응답
            return Response(
                stream_with_context(stream_summary(lecture_id)),
...
def quizzes_api():
    ...
    if request.method == 'POST':
        ...
        if lecture_id not in global_summary or not global_summary[lecture_id]:
            ...
            # 그래도 없으면 에러 반환
            if lecture_id not in global_summary or not global_summary[lecture_id]:
                return jsonify({'error': '요약 결과가 없습니다. 먼저 /summary 엔드포인트를 호출하세요.'}), 400
        ...
        if streaming:
            ...
            try:
                ...
                return jsonify({"success": True, "quiz": quiz_text})
            except Exception as e:
                return jsonify({"error": str(e)}), 500
    else:
